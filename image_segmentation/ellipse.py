'''
Taken from https://github.com/bdhammel/least-squares-ellipse-fitting/blob/master/ellipse.py
'''
import numpy as np
    ...
    def _check_data(self, X):
        ...
        if not n_features == self.ALLOWED_FEATURES:
            raise ValueError("Incorrect number of features. "
                             "Got {} features, expected 2. ".format(n_features))

        if n_samples < 5:
            raise ValueError("Received too few samples"
                             "Got {} features, 5 or more required. ".format(n_samples))
