...
class ActiveContourSegmentation(object):
    ...
    with all the considerations of the paper (weights update, all the lambda
    weights, ellipse distance functions, models, etc).
    '''
    ...
    def active_cont_segm(self, I, init_mask):
        ...
        for its in range(self.max_iter):
            ...
            curvature = self.get_real_curvature(normalized_grad, idx)
            # model_match = M_1 - M_2
            ...
            try:
                dphidt = \
                    (model_match) - \
                    (second_term) - \
                    (np.multiply(potential_func_P[idx[0], idx[1]], curvature))
            except TypeError:
    ...
    def get_real_curvature(self, grad, idx):
        '''
        ...
        return np.array(der_grad_x + der_grad_y)[idx[0], idx[1]]
    ...
    def clean_image(self, BW, acum):
        ...
        if area:
            ...
            # Line 431 has been commented and replaced by 430 because
            # it will modify the shape of several protate zones in a
            # way that is undersirable.
            acum = holes_filled
